(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.jltiusl-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.jltiusl-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.jltiusl-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.jltiusl-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: JLTIUSLCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jltiusl_recommended_activate_plugin",
            file: file,
            nonce: JLTIUSLCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jltiusl_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTIUSLCORE.recommended_nonce
        },
