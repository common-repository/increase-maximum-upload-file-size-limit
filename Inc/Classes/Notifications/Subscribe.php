...
if ( ! class_exists( 'Subscribe' ) ) {
	...
	class Subscribe extends Notice {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'wp_ajax_jltiusl_subscribe', array( $this, 'jltiusl_subscribe' ) );
		}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jltiusl_subscribe() {
			check_ajax_referer( 'jltiusl_subscribe_nonce' );
		...
		public function notice_content() {
			...
			?>
			<div class="notice notice-jltiusl is-dismissible notice-plugin-review notice-<?php echo esc_attr( $this->color ); ?> jltiusl-notice-<?php echo esc_attr( $this->get_id() ); ?>">
				<button type="button" class="notice-dismiss jltiusl-notice-dismiss"></button>
				<img width="70" height="70" src="<?php echo esc_url( JLTIUSL_IMAGES . '/logo.png' ); ?>" alt="<?php esc_attr_e( 'Logo', 'increase-maximum-upload-file-size-limit' ); ?>">
				<div class="jltiusl-subscribe-content">
					<?php $this->set_title(); ?>
					<form style="display:flex">
						<div class="jltiusl-plugin-subscribe-input">
						...
						<button type="submit" class="button button-primary jltiusl-subscribe-btn"><?php esc_html_e( 'Get Discount', 'increase-maximum-upload-file-size-limit' ); ?></button>
					</form>
				</div>
			</div>
			<?php
		...
		public function core_script( $trigger_time ) {
			...
			<script>
				jQuery(document).on('submit', '.jltiusl-notice-subscribe .jltiusl-subscribe-content form', function(e) {
					...
					let formWrapper = form.closest('.jltiusl-subscribe-content');
					...
					jQuery.ajax({
							...
							crossDomain: true,
							data: {
								action: 'jltiusl_subscribe',
								_wpnonce: '<?php echo esc_js( wp_create_nonce( 'jltiusl_subscribe_nonce' ) ); ?>',
								name: name,
								email: email,
							}
						})
						.done(function(response) {
							formWrapper.hide().after('<p class="jltiusl--notice-message"><strong>' + response.data + '</strong><p>');
							let subsTimeout = setTimeout(function() {
								jltiusl_notice_action(null, form, 'disable');
