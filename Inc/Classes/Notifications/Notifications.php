...
class Notifications {
	...
	public function __construct() {
		...
		add_action( 'jltiusl_display_notice', array( $this, 'display_notice' ), 10, 2 );
		add_action( 'jltiusl_display_popup', array( $this, 'display_popup' ), 10, 2 );

		add_action( 'wp_ajax_jltiusl_notification_action', array( $this, 'notification_action' ) );
	}
	...
	public function notification_action() {
		check_ajax_referer( 'jltiusl_notification_nonce' );
	...
	public function setup_notifications_by_type( $type ) {
		...
		// Block if necessary .
		$notification_last_fired = get_option( "jltiusl_{$type}_last_interact" );
		...
		do_action( "jltiusl_display_{$type}", $notification, $trigger_time );
	}
	...
	public function display_popup( $popup, $trigger_time ) {
		...
		<div class="jltiusl-popup" id="jltiusl-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1">

			<div class="jltiusl-popup-overlay"></div>

			<div class="jltiusl-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --jltiusl-popup-color: <?php echo esc_attr( $popup->get_content( 'btn_color' ) ); ?>;">
				...
				<div class="jltiusl-popup-modal-footer">
					...
					<div class="jltiusl-popup-countdown" style="display: none;">
						<span class="jltiusl-popup-countdown-text"><?php echo esc_html__( 'Deal Ends In', 'increase-maximum-upload-file-size-limit' ); ?></span>
						<div class="jltiusl-popup-countdown-time">
							<div>
								<span data-counter="days">00</span>
								<span><?php echo esc_html__( 'Days', 'increase-maximum-upload-file-size-limit' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="hours">00</span>
								<span><?php echo esc_html__( 'Hours', 'increase-maximum-upload-file-size-limit' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="minutes">00</span>
								<span><?php echo esc_html__( 'Minutes', 'increase-maximum-upload-file-size-limit' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="seconds">00</span>
								<span><?php echo esc_html__( 'Seconds', 'increase-maximum-upload-file-size-limit' ); ?></span>
							</div>
						</div>
					</div>

					<!-- button  -->
					<a class="jltiusl-popup-button" target="_blank" href="<?php echo esc_url( $popup->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $popup->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			function jltiusl_popup_action(evt, $this, $action_type) {

				evt.preventDefault();

				$this.closest('.jltiusl-popup').fadeOut(200);
			...
			// Notice Dismiss
			jQuery('body').on('click', '.jltiusl-popup .popup-dismiss', function(evt) {
				jltiusl_popup_action(evt, jQuery(this), 'dismiss');
			});

			// Notice Disable
			jQuery('body').on('click', '.jltiusl-popup .popup-disable', function(evt) {
				jltiusl_popup_action(evt, jQuery(this), 'disable');
			});
		</script>
